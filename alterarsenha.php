...
$confirmar_nova = $_REQUEST['confirmar_nova'];  	
$texto = '';
$erros = false;
$string = strlen($senha_nova);
   if ($senha_nova == '')
   {
	   $texto = $texto.'A senha não pode estar em branco!\n';
	   $erros = true;
   }
   	elseif ($senha_nova == $senha) 
   	{
   		$texto = $texto.'A senha não pode ser a mesma da anterior!\n';
		$erros = true;
   	}
   
     else if ($string < 8)
      {
     	 $texto = $texto.'A senha não pode ter menos de 8 caracteres!\n';
	     $erros = true;
     }
     if ($confirmar_nova == '') {
     	$texto = $texto.'Confirmar senha não pode estar em branco!\n';
	    $erros = true;
     }
     elseif ($senha_nova != $confirmar_nova)
	   {
		 $texto=  $texto.'Senhas diferentes!\n';
		 $erros = true;   
	   }
     if (!$erros){
     	...
     	if (conecta()) {
     		...
     		$sql = "UPDATE cad_usuario SET senha='$senha_nova' WHERE usuario = '$usuario'";
...
<tr>	
 ...
 <tr>	
		<td width="20%" bgcolor="#C0C0C0" align="center" valign="top" bgcolor="#C0C0C0" cellspacing="0">
			<form id="form_nova" name="form_nova" method="post" action= "alterarsenha.php?op=3">
				...
				<table width="50%" height="300px" border="0">
					...
					<tr height="70">
						<td align="center"><input type="password" name="senha_nova" id="senha_nova"></td>
						<td align="center"><input type="password" name="confirmar_nova" id="confirmar_nova"></td>
