...
$sql4= "SELECT email FROM cad_usuario WHERE usuario = '$usuario'";
...
$erros = false;
     ...
     if ($email_novo == '') {
     	$texto = $texto.'O email não pode estar em branco!\n';
	    $erros = true;
     }
     elseif (!filter_var($email_novo, FILTER_VALIDATE_EMAIL)) {
      $texto = $texto.'Email não é valido!\n';
      $erros = true;
    }
    elseif ($email == $email_novo) {
      $texto = $texto.'Email não pode ser o mesmo!\n';
      $erros = true;
    }
...
$sql4= "SELECT email FROM cad_usuario where email ='$email_novo'";
	$recordset2=mysql_query($sql4); // fazer o query a base de dados
	$record2=mysql_fetch_assoc($recordset2); // ler o registo do recordset
	$email=$record2['email'];
	if ($email_novo == $email) {
		alert("Email ja cadastrado!");
     		...
     		$sql = "UPDATE cad_usuario SET email='$email_novo' WHERE usuario = '$usuario'";
...
<tr>	
 ...
 <tr>	
		<td width="20%" bgcolor="#C0C0C0" align="center" valign="top" bgcolor="#C0C0C0" cellspacing="0">
			<form id="form_nova" name="form_nova" method="post" action= "alteraremail.php?op=3">
				...
				<table width="50%" height="300px" border="0">
					...
					<tr height="70">
						<td align="center"><input type="text" name="email_novo" id="email_novo"></td>
